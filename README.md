...
アテンション・エコノミーは終わりつつある。しかし、次に何が来るのかは、誰にも分かっていない。

本書は、その「次」を描くための本だ。完成されたモデルを提示する本ではない。**設計条件（Design Brief）を提示する本だ。**

アテンション・エコノミーの構造的解剖から始め、法廷が「設計責任」という新しい法的概念を確立した経緯、1兆ドル規模の見えない社会的コスト、アテンション格差という新しい不平等の軸、FacebookのMSI（Meaningful Social Interactions）が証明した「指標の罠」、移行期の実験と失敗、クリエイター報酬の再設計、自己決定理論（SDT）に基づく「真のエンゲージメント」の定義を経て、次世代SNSの6つの設計条件を提示する。

**本書は答えを書かない。設計図を書く。次世代SNSをつくるのは、この設計条件を読んだあなたかもしれない。**
...
| ファイル | 言語 | 内容 |
| --- | --- | --- |
| [the-attention-economy-is-over_JP.md](./docs/jp/the-attention-economy-is-over_JP.md) | 🇯🇵 日本語 | 本文（日本語版） |
| [the-attention-economy-is-over_EN.md](./docs/en/the-attention-economy-is-over_EN.md) | 🇺🇸 English | 本文（英語版） |
...
- **第1章:** アテンション・エコノミーの解剖
- **第2章:** 法廷が「設計」を裁いた — コンテンツ責任から設計責任へ
- **第3章:** 1兆ドルの見えないコスト
- **第4章:** アテンション税 — 新しい格差の軸
- **第5章:** Facebookが証明した「指標の罠」— MSIの失敗
- **第6章:** 移行期の実験 — 何が動き、何が失敗したか
- **第7章:** クリエイター報酬の再設計
- **第8章:** 「真のエンゲージメント」とは何か — 自己決定理論（SDT）
...
| **A Trillion Dollars and a Firebomb** | AI時代の同時加速する現実。1兆ドルと火炎瓶が同じ構造から生まれる非対称を描く | [GitHub](https://github.com/Leading-AI-IO/a-trillion-and-a-firebomb) |
...
Issues and Pull Requests are welcome. 本書の構造分析に対するフィードバック、各国・地域のSNS規制動向の追加情報、誤字脱字の修正など、皆様からのContributeを歓迎します。
